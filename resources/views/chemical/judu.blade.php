...
<div class="bjui-row col-2">
    ...
    <label class="row-label">存储条件</label>
    <div class="row-input required">
        <input type="text" name="{{ $item->id }}存储条件" value="{{ $item->存储条件 }}" data-rule="required">
    </div>
    <label class="row-label">是否满足安全存放条件</label>
    <div class="row-input required">
        <input type="text" name="{{ $item->id }}是否满足安全存放条件" value="{{ $item->是否满足安全存放条件 }}" data-rule="required">
    </div>
    <label class="row-label">科研项目名称</label>
    <div class="row-input required">
        <input type="text" name="{{ $item->id }}科研项目名称" value="{{ $item->科研项目名称 }}" data-rule="required">
    </div>
    <label class="row-label">科研项目编号</label>
    <div class="row-input required">
        <input type="text" name="{{ $item->id }}科研项目编号" value="{{ $item->科研项目编号 }}" data-rule="required">
    </div>
</div>
